export type NativeMediaBackend = "swift-sidecar" | "bun-fallback" | "gstreamer" | "ffmpeg" | "ffprobe" | "videoflow-server" | "bun-media-server";
...
export async function requestNativeMedia<T = unknown>(
  ...
  if (task !== "transcribe-audio" && task !== "gstreamer-normalize" && task !== "probe-media" && task !== "normalize-media" && task !== "extract-audio" && task !== "store-media" && task !== "media-status" && task !== "render-video-server" && hasSwiftSidecarBridge()) {
    try {
